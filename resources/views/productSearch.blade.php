                            ...
                            <div class="products-category">
                                ...
                                <div id="mfilter-content-container">
                                    <div class="banner-box">
                                        <p>Kết quả tìm kiếm cho {{request('search_term', $searchTerm??'')}}</p>
                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="short-by-show form-inline text-center  col-md-12 col-sm-12">
                                                <div class="form-group sort-by pull-right">
                                                    <select id="input-sort" class="form-control" onchange="window.location.href=this.value;">
                                                        <option value="">Sắp xếp: Mặc định</option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'name', 'sort_direction' => 'asc'])) }}" {{ $sortBy == 'name' && $sortDirection == 'asc' ? 'selected' : '' }}>
                                                            Sắp xếp: Tên (A - Z)
                                                        </option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'name', 'sort_direction' => 'desc'])) }}" {{ $sortBy == 'name' && $sortDirection == 'desc' ? 'selected' : '' }}>
                                                            Sắp xếp: Tên (Z - A)
                                                        </option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'sale_price', 'sort_direction' => 'asc'])) }}" {{ $sortBy == 'sale_price' && $sortDirection == 'asc' ? 'selected' : '' }}>
                                                            Sắp xếp: Giá (Thấp > Cao)
                                                        </option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'sale_price', 'sort_direction' => 'desc'])) }}" {{ $sortBy == 'sale_price' && $sortDirection == 'desc' ? 'selected' : '' }}>
                                                            Sắp xếp: Giá (Cao > Thấp)
                                                        </option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'rating', 'sort_direction' => 'desc'])) }}" {{ $sortBy == 'rating' && $sortDirection == 'desc' ? 'selected' : '' }}>
                                                            Sắp xếp: Đánh giá (Cao nhất)
                                                        </option>
                                                        <option value="{{ route('search', array_merge(request()->query(), ['sort_by' => 'rating', 'sort_direction' => 'asc'])) }}" {{ $sortBy == 'rating' && $sortDirection == 'asc' ? 'selected' : '' }}>
                                                ...
                                                <div class="product-thumb">
                                                    <div class="product-item-content" style="height: 372px;">
                                                        <div class="left-block"><a
                                                                href="{{ route('homeProducts.displayProductDetail',['parentCategorySlug' => $product->category->parent->slug,'childCategorySlug' => $product->category->slug,'productSlug' => $product->slug]) }}"
                                                            >
